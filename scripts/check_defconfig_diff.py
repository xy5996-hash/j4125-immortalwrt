...
from profile_rules import expand_feature_kconfig, expand_feature_packages, feature_map, unique
...
def main() -> int:
    ...
    for symbol, value in expand_feature_kconfig(data).items():
        ...
        if expected not in after_lines:
            errors.append(f"required config missing after defconfig: {expected}")
